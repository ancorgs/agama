...
function removeDevice(apiModel: apiModel.Config, deviceName: string): apiModel.Config {
  apiModel.drives = apiModel.drives.filter((d) => d.name !== deviceName);
  apiModel.mdRaids = apiModel.mdRaids.filter((d) => d.name !== deviceName);
  return apiModel;
}

function setBoot(model: model.Model, apiModel: apiModel.Config, boot: apiModel.Boot) {
  const bootDevice = model.boot.getDevice();
  if (bootDevice && !bootDevice.isUsed) removeDevice(apiModel, bootDevice.name);
