  ...
  const text = (drive: type.DriveElement): string => {
    ...
    if (driveUtils.hasFilesystem(drive)) {
      if (driveUtils.hasPv(drive)) {
        if (drive.boot) {
          // TRANSLATORS: %s will be replaced by the device name and its size - "/dev/sda, 20 GiB"
          return _("Use %s for additional partitions, LVM and booting");
        }
        // TRANSLATORS: %s will be replaced by the device name and its size - "/dev/sda, 20 GiB"
        return _("Use %s for additional partitions and LVM");
      }
    ...
    <MenuContainer
      ...
      toggle={
        <MenuToggle
          ...
          className="menu-toggle-inline"
        >
          <b>{_("No additional partitions will be created")}</b>
        </MenuToggle>
      }
      menuRef={menuRef}
      menu={
